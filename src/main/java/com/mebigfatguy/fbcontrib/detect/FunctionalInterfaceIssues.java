...
public class FunctionalInterfaceIssues extends BytecodeScanningDetector {
	...
	private static final FQMethod TOLIST = new FQMethod("java/util/stream/Collectors", "toList",
	...
	private static final FQMethod MAP = new FQMethod("java/util/stream/Stream", "map",
			"(Ljava/util/function/Function;)Ljava/util/stream/Stream;");
	private static final FQMethod LIMIT = new FQMethod("java/util/stream/Stream", "limit",
				...
				if (bootstrapAtt != null) {
					...
					for (Map.Entry<String, List<FIInfo>> entry : functionalInterfaceInfo.entrySet()) {
						for (FIInfo fii : entry.getValue()) {
							...
							if (bugType != null) {
								bugReporter
										.reportBug(new BugInstance(this, bugType.name(), NORMAL_PRIORITY).addClass(this)
							...
							if (stack.getStackDepth() >= 2) {
								...
								if (FIIUserValue.MAP_ITEM == itm.getUserValue()) {
									bugReporter.reportBug(new BugInstance(this,
											BugType.FII_AVOID_MAP_BEFORE_LIMIT.name(), NORMAL_PRIORITY).addClass(this)
					...
					if (stack.getStackDepth() > 0) {
						...
						if (uv == FIIUserValue.COLLECTION_STREAM_ITEM) {
							...
							if (TOLIST.equals(fqm) || TOSET.equals(fqm)) {
								bugReporter.reportBug(
										new BugInstance(this, BugType.FII_USE_COPYCONSTRUCTOR.name(), NORMAL_PRIORITY)
												.addClass(this).addMethod(this).addSourceLine(this));
							}
						}
					}
