...
public class ArrayIndexOutOfBounds extends BytecodeScanningDetector {
	...
	public void visitCode(Code obj) {
		...
		for (Integer pc : nullStoreToLocation.values()) {
			bugReporter
					.reportBug(new BugInstance(this, BugType.AIOB_ARRAY_STORE_TO_NULL_REFERENCE.name(), HIGH_PRIORITY)
			...
			if (index != null) {
				...
				if ((sz != null) && (index.intValue() >= sz.intValue())) {
					bugReporter.reportBug(
							new BugInstance(this, BugType.AIOB_ARRAY_INDEX_OUT_OF_BOUNDS.name(), HIGH_PRIORITY)
