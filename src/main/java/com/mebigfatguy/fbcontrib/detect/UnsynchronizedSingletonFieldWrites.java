...
/**
 * looks for field writes to objects that are non singletons, where the write is
 * not synchronized
 */
public class UnsynchronizedSingletonFieldWrites extends BytecodeScanningDetector {
    ...
    private static final Set<String> SPRING_CLASS_ANNOTATIONS = UnmodifiableSet.create(
            ...
            "Lorg/springframework/stereotype/Service;", "Lorg/springframework/stereotype/Controller;"
    // @formatter:on
    );
    ...
    /**
     * implements the visitor to look for methods that could possibly be 'normal'
     * methods where a field is written to
     *
    ...
    public void sawOpcode(int seen) {
        try {
            ...
            case Const.MONITOREXIT:
                syncBlockCount--;
                // javac stutters the exit in catch/finally blocks in a bolux of nastyness, so
                // guard against it
                if (syncBlockCount < 0) {
                    syncBlockCount = 0;
                }
                    ...
                    if (stack.getStackDepth() >= 2) {
                        ...
                        if (itm.getRegisterNumber() == 0) {
                            bugReporter.reportBug(
                                    new BugInstance(this, BugType.USFW_UNSYNCHRONIZED_SINGLETON_FIELD_WRITES.name(),
    ...
    /**
     * looks for methods that should not be scanned for fields writes for a variety
     * of reasons
     * <ul>
     * <li>Constructor</li>
