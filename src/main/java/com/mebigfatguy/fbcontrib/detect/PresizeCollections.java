...
public class PresizeCollections extends BytecodeScanningDetector {
    ...
    private static final FQMethod ITERATOR_HASNEXT = new FQMethod("java/util/Iterator", "hasNext",
    ...
    private static final FQMethod HASHMAP_SIZED_CTOR = new FQMethod("java/util/HashMap", "<init>",
            SignatureBuilder.SIG_INT_TO_VOID);
    private static final FQMethod HASHSET_SIZED_CTOR = new FQMethod("java/util/HashSet", "<init>",
            SignatureBuilder.SIG_INT_TO_VOID);
    ...
    public void sawOpcode(int seen) {
        ...
        try {
                            ...
                            if (HASHMAP_SIZED_CTOR.equals(fqMethod) || HASHSET_SIZED_CTOR.equals(fqMethod)) {
                                ...
                                if ((xm != null) && "size".equals(xm.getMethodDescriptor().getName())) {
                                    bugReporter.reportBug(new BugInstance(this,
                                            BugType.PSC_SUBOPTIMAL_COLLECTION_SIZING.name(), NORMAL_PRIORITY)
            ...
            case Const.INVOKEINTERFACE:
                ...
                } else if (ITERATOR_HASNEXT
                        .equals(new FQMethod(getClassConstantOperand(), methodName, getSigConstantOperand()))) {
                    if (stack.getStackDepth() > 0) {
                        OpcodeStack.Item itm = stack.getStackItem(0);
     ...
     *
     * @param c the currently parsed code object
     */
    ...
    /**
     * returns if the conditional is based on a method call from an object that has
     * no sizing to determine what presize should be. it's possible the correct
     * implementation should just return true, if
     * <code>if ((seen != IFNE) || (stack.getStackDepth() == 0))</code>
     *
     * @param seen the current visited opcode
