...
public class StringifiedTypes extends BytecodeScanningDetector {
	...
	static {
		...
		String objectToInt = new SignatureBuilder().withParamTypes(Values.SLASHED_JAVA_LANG_OBJECT)
				.withReturnType(Values.SIG_PRIMITIVE_INT).toString();

		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_LIST, "contains", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN), parm0);
		COLLECTION_PARMS.put(new FQMethod(Values.SLASHED_JAVA_UTIL_LIST, "add", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN),
				parm0);
		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_LIST, "remove", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN), parm0);
		COLLECTION_PARMS.put(new FQMethod(Values.SLASHED_JAVA_UTIL_LIST, "set",
				...
				parm0N1);
		COLLECTION_PARMS.put(new FQMethod(Values.SLASHED_JAVA_UTIL_LIST, "add", new SignatureBuilder()
				.withParamTypes(Values.SIG_PRIMITIVE_INT, Values.SLASHED_JAVA_LANG_OBJECT).toString()), parm0);
		...
		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_SET, "contains", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN), parm0);
		COLLECTION_PARMS.put(new FQMethod(Values.SLASHED_JAVA_UTIL_SET, "add", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN),
				parm0);
		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_SET, "remove", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN), parm0);

		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_MAP, "containsKey", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN),
				parm0);
		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_MAP, "containsValue", SignatureBuilder.SIG_OBJECT_TO_BOOLEAN),
				parm0);
		COLLECTION_PARMS.put(new FQMethod(Values.SLASHED_JAVA_UTIL_MAP, "get", SignatureBuilder.SIG_OBJECT_TO_OBJECT),
				parm0N1);
		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_MAP, "put", SignatureBuilder.SIG_TWO_OBJECTS_TO_OBJECT),
				parm01N1);
		COLLECTION_PARMS.put(
				new FQMethod(Values.SLASHED_JAVA_UTIL_MAP, "remove", SignatureBuilder.SIG_OBJECT_TO_OBJECT), parm0N1);
	}
	...
	private static final FQMethod MAP_PUT = new FQMethod(Values.SLASHED_JAVA_UTIL_MAP, "put",
			SignatureBuilder.SIG_TWO_OBJECTS_TO_OBJECT);
					...
					if (priority != null) {
						...
						if (stackDepth > numParameters) {
							...
							if ((item.getXField() != null) || FROM_FIELD.equals(item.getUserValue())) {
								bugReporter.reportBug(new BugInstance(this, BugType.STT_STRING_PARSING_A_FIELD.name(),
						...
						if (item.getXField() == null) {
								...
								if (xm != null) {
									if (Values.DOTTED_JAVA_LANG_STRINGBUILDER.equals(xm.getClassName())) {
										bugReporter.reportBug(new BugInstance(this,
												BugType.STT_TOSTRING_MAP_KEYING.name(), NORMAL_PRIORITY).addClass(this)
							...
							for (int parm : checkParms) {
								if ((parm >= 0) && Values.TOSTRING.equals(stack.getStackItem(parm).getUserValue())) {
									bugReporter.reportBug(new BugInstance(this,
											BugType.STT_TOSTRING_STORED_IN_FIELD.name(), NORMAL_PRIORITY).addClass(this)
													.addMethod(this).addSourceLine(this));
									break;
								}
							}
				...
				if (stackDepth > 0) {
					...
					if (Values.TOSTRING.equals(item.getUserValue())) {
						bugReporter.reportBug(
								new BugInstance(this, BugType.STT_TOSTRING_STORED_IN_FIELD.name(), NORMAL_PRIORITY)
