...
/**
 * Detects use of backport libraries, when the code in question is compiled in a
 * jdk that has the functionality available. Libraries include:
 * <ul>
 * <li>Use of Backport concurrent classes. Updated/Efficient version of these
 * classes are available in versions of the JDK 1.5 and higher, and these
 * classes should only be used if you are targeting JDK 1.4 and lower.</li>
 * <li>Use of ThreeTen time backport classes, instead of java.time packages
 * which are now available in JDK 1.8 and higher, and these classes should only
 * be used if you are targeting JDK 1.7 and lower
 * </ul>
 */
public class BackportReusePublicIdentifiers extends OpcodeStackDetector {
    ...
    /**
     * overrides the visitor to look for method calls to the emory backport
     * concurrent library, or threeten bp library when the now built-in versions are
     * available
     *
    ...
    public void sawOpcode(int seen) {
        ...
        case Const.INVOKESPECIAL: {
            ...
            for (Backports backport : BACKPORTS) {
                if (Values.CONSTRUCTOR.equals(methodName) && className.startsWith(backport.getPathPrefix())
                        && (clsVersion >= backport.getMinimumJDK())) {
                    reportBug(backport.getLibrary());
                    break;
                }
            }
     ...
     *
     * @param library the type of backport library that is in use
     */
    private void reportBug(Library library) {
        bugReporter
                .reportBug(new BugInstance(this, BugType.BRPI_BACKPORT_REUSE_PUBLIC_IDENTIFIERS.name(), NORMAL_PRIORITY)
                        .addClass(this).addMethod(this).addSourceLine(this).addString(library.name()));
    }

    /**
     * describes a library that is a backport of a package that is now included in
     * the jdk by default
     */
