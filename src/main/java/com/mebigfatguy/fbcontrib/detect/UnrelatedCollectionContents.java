...
public class UnrelatedCollectionContents extends BytecodeScanningDetector {
    private static final Set<String> COLLECTION_CLASSES = UnmodifiableSet.create(Values.SLASHED_JAVA_UTIL_COLLECTION,
            Values.SLASHED_JAVA_UTIL_LIST, Values.SLASHED_JAVA_UTIL_MAP, Values.SLASHED_JAVA_UTIL_SET,
    ...
    /**
     * implements the visitor to reset the opcode stack, and clear the various
     * collections
     *
    ...
    /**
     * implements the visitor to look for collection method calls that put objects
     * into the collection that are unrelated by anything besides java.lang.Objecct
     *
    ...
    /**
     * processes an add into a collection, by processing all the super
     * classes/interfaces of an object and removing the possible set of parent
     * classes that have been seen so far, by doing what amounts to a intersection
     * of what has been seen before, and this occurance.
     *
     * @param colItm the collection that is being added to
     * @param addItm the added item
    ...
    private void checkAdd(final OpcodeStack.Item colItm, final OpcodeStack.Item addItm) throws ClassNotFoundException {
        ...
        if (reg == -1) {
            ...
            if (commonSupers == null) {
                ...
                addNewItem(commonSupers, addItm);
                Integer scopeEnd = Integer.valueOf(
    ...
    /**
     * intersects the set of possible superclass that this collection might have
     * seen before with this one. If we find that there is no commonality between
     * superclasses, report it as a bug.
     *
     * @param supers the collection of possible superclass/interfaces that has been
     *               seen for this collection thus far
     * @param sla    the location of this add
     * @param addItm the currently added item
     * @throws ClassNotFoundException if a superclass/interface can not be found
     */
    ...
    /**
     * adds this item's type and all of it's superclass/interfaces to the set of
     * possible types that could define this added item
     *
     * @param supers the current set of superclass items
     * @param addItm the item we are adding
     * @throws ClassNotFoundException if a superclass/interface is not found
     */
    ...
    /**
     * performs a typical set intersection between what types of possible
     * superclasses/interfaces has been seen before, for this collection, and what
     * is now seen.
     *
     * @param orig the existing set of superclasses used for this collection
     * @param add  the currently added item
     */
