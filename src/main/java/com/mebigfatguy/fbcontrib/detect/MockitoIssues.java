		...
		try {
			...
			outer: for (AnnotationEntry ann : cls.getAnnotationEntries()) {
				if (ann.isRuntimeVisible()) {
					if ("org.junit.jupiter.api.extension.ExtendWith"
							.equals(SignatureUtils.stripSignature(ann.getAnnotationType()))) {
					...
					} else if ("org.testng.annotations.Listeners"
							.equals(SignatureUtils.stripSignature(ann.getAnnotationType()))) {
						for (ElementValuePair evp : ann.getElementValuePairs()) {
							if ("value".equals(evp.getNameString()) && evp.getValue() instanceof ArrayElementValue) {
								ArrayElementValue aev = (ArrayElementValue) evp.getValue();
		...
		try {
			...
			case Const.INVOKESTATIC: {
					...
					if ("org/mockito/MockitoAnnotations".equals(clsName)) {
						...
						if ("openMocks".equals(methodName)) {
							bugReporter.reportBug(
									new BugInstance(this, BugType.MK_UNNEEDED_OPENMOCKS.name(), NORMAL_PRIORITY)
											.addClass(this).addMethod(this).addSourceLine(this));
						}
					} else if ("org/mockito/Mockito".equals(clsName)) {
						String methodName = getNameConstantOperand();
						if ("validateMockitoUsage".equals(methodName)) {
							bugReporter.reportBug(new BugInstance(this, BugType.MK_UNNEEDED_VALIDATEMOCKITOUSAGE.name(),
				...
				if ("org/mockito/Mockito".equals(clsName)) {
					...
					boolean isClassParm = false;
					if (numParms == 1) {
						List<String> sigs = SignatureUtils.getParameterSignatures(sig);
						isClassParm = "Ljava/lang/Class;".equals(sigs.get(0));
					}

					if ("mock".equals(methodName) && isClassParm) {
						userValue = MKUserValue.MOCK;
					} else if ("spy".equals(methodName) && isClassParm) {
						...
						if (stack.getStackDepth() > 0) {
							...
							if (itm.getUserValue() == MKUserValue.SPIED_OBJ) {
								bugReporter.reportBug(
										new BugInstance(this, BugType.MK_USE_DORETURN.name(), NORMAL_PRIORITY)
												.addClass(this).addMethod(this).addSourceLine(this));
							}
						}
						...
						if (userValue == MKUserValue.CAPTOR) {
							bugReporter
									.reportBug(new BugInstance(this, BugType.MK_CAPTOR_IN_WHEN.name(), NORMAL_PRIORITY)
											.addClass(this).addMethod(this).addSourceLine(this));
						}
					}
				} else {
				...
				if (sawAnnotatedField) {
					...
					if (clsName.equals(cls.getClassName())) {
						...
						if (mockedFields.contains(fieldName)) {
							bugReporter.reportBug(
									new BugInstance(this, BugType.MK_MOCKED_FIELD_REASSIGNED.name(), NORMAL_PRIORITY)
											.addClass(this).addMethod(this).addSourceLine(this));
							return;
						} else if (spiedFields.contains(fieldName)) {
							bugReporter.reportBug(
									new BugInstance(this, BugType.MK_SPIED_FIELD_REASSIGNED.name(), NORMAL_PRIORITY)
											.addClass(this).addMethod(this).addSourceLine(this));
							return;
						}
						...
						if (inBeforeEach) {
							if (stack.getStackDepth() > 0) {
								...
								if (itm.getUserValue() == MKUserValue.MOCK) {
									bugReporter.reportBug(
											new BugInstance(this, BugType.MK_MOCKED_FIELD_IN_CODE.name(), LOW_PRIORITY)
													.addClass(this).addMethod(this).addSourceLine(this));
								} else if (itm.getUserValue() == MKUserValue.SPY) {
									bugReporter.reportBug(
											new BugInstance(this, BugType.MK_SPIED_FIELD_IN_CODE.name(), LOW_PRIORITY)
													.addClass(this).addMethod(this).addSourceLine(this));
								}
							}
						}
