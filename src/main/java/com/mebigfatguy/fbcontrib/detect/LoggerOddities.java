...
public class LoggerOddities extends BytecodeScanningDetector {
	...
	private static final Pattern BAD_STRING_FORMAT_PATTERN = Pattern.compile(
		...
		try {
			...
			if ((seen == Const.LDC) || (seen == Const.LDC_W)) {
				...
				if (Values.SLASHED_JAVA_LANG_STRING.equals(getClassConstantOperand())
						&& "format".equals(getNameConstantOperand()) && (stack.getStackDepth() >= 2)) {
			...
			if (SIG_STRING_AND_THROWABLE_TO_VOID.equals(sig) || SIG_OBJECT_AND_THROWABLE_TO_VOID.equals(sig)) {
				if (stack.getStackDepth() >= 2) {
					...
					if ((uv != null) && (uv.getType() == LOUserValue.LOType.MESSAGE_REG)
						...
						bugReporter
								.reportBug(new BugInstance(this, BugType.LO_STUTTERED_MESSAGE.name(), NORMAL_PRIORITY)
				...
				if (stack.getStackDepth() > 0) {
					...
					if ((clazz != null) && clazz.instanceOf(throwableClass)) {
						bugReporter.reportBug(new BugInstance(this, BugType.LO_LOGGER_LOST_EXCEPTION_STACK_TRACE.name(),
				...
				if (SignatureBuilder.SIG_STRING_TO_VOID.equals(signature)
					...
					if (stack.getStackDepth() >= numParms) {
						...
						if (con instanceof String) {
							...
							if (m.find()) {
								bugReporter.reportBug(new BugInstance(this, BugType.LO_INVALID_FORMATTING_ANCHOR.name(),
										NORMAL_PRIORITY).addClass(this).addMethod(this).addSourceLine(this));
							} else {
								m = BAD_STRING_FORMAT_PATTERN.matcher((String) con);
								if (m.find()) {
									bugReporter.reportBug(new BugInstance(this,
											BugType.LO_INVALID_STRING_FORMAT_NOTATION.name(), NORMAL_PRIORITY)
													.addClass(this).addMethod(this).addSourceLine(this));
								} else {
									...
									if (actualParms != -1) {
										...
										if ((!hasEx && (expectedParms != actualParms))
												|| (hasEx && ((expectedParms != (actualParms - 1))
														&& (expectedParms != actualParms)))) {
											bugReporter
													.reportBug(
															new BugInstance(this,
																	BugType.LO_INCORRECT_NUMBER_OF_ANCHOR_PARAMETERS
																			.name(),
							...
							if ((uv != null) && (uv.getType() == LOUserValue.LOType.METHOD_NAME)
								...
								bugReporter.reportBug(
										new BugInstance(this, BugType.LO_APPENDED_STRING_IN_FORMAT_STRING.name(),
												NORMAL_PRIORITY).addClass(this).addMethod(this).addSourceLine(this));
							} else {
								...
								if ((uv != null) && (uv.getType() == LOUserValue.LOType.SIMPLE_FORMAT)) {
									bugReporter
											.reportBug(
													new BugInstance(this,
															BugType.LO_EMBEDDED_SIMPLE_STRING_FORMAT_IN_FORMAT_STRING
																	.name(),
															NORMAL_PRIORITY).addClass(this).addMethod(this)
																	.addSourceLine(this));
								}
	...
	/**
	 * returns the number of parameters slf4j is expecting to inject into the format
	 * string
	 *
	 * @param signature
