...
4. There are two branches, findbugs and spotbugs. Typically you will want to develop using the SpotBugs directory. Any merge requests you make should be against this branch.
5. Open Eclipse.  File>Import and then choose "Existing projects into workspace", and find the fb-contrib folder you created in step 3.  Ignore any compile errors (for now).
6. Install the latest SpotBugs release from here https://github.com/spotbugs/spotbugs/releases, choose either the zip or tgz formats, typically.
7. In the fb-contrib project, find the `user.properties.example` file.  Make a copy of it named user.properties (this will not be tracked by version control). Modify the spotbugs.dir property to where ever you have the SpotBugs distribution installed.  This is the [executable SpotBugsfolder, *not* a source folder.  The jar will be "installed" to (spotbugs.dir)\plugin.
...
9. add remote debugging of SpotBugs (add something like: java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:7777 -jar /home/dave/apps/spotbugs-4.9.7/lib/spotbugs.jar) and then in eclipse set up a remote debug configuration pointing to the port you specified)
...
dependencies {
    ...
    findbugsPlugins 'com.mebigfatguy.fb-contrib:fb-contrib:7.7.3'
}
